---
id: KB-RU-000049
title: "7206-2RS — подшипник шариковый радиально-упорный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["7206 2rs", "7206 BEP", "7206B"]
designation:
  base: "7206"
  suffixes: ["2RS"]
...
equivalents:
  - {manufacturer: "SKF", code: "7206 BEP"}
  - {manufacturer: "FAG", code: "7206-B-TVP"}
  - {manufacturer: "NTN", code: "7206B"}
  - {manufacturer: "NSK", code: "7206B"}
  - {manufacturer: "ГОСТ", code: "36206"}
load_capacity:
  dynamic_C_kN: 20.3
  static_C0_kN: 12.5
  rpm_limit: 8800
...
# 7206-2RS — подшипник шариковый радиально-упорный
...
- SKF: 7206-2RSH (H = Heavy duty уплотнение)
- FAG: 7206.2RSR (точка вместо дефиса)
- NTN: 7206LLU (LLU = низкое трение)
- NSK: 7206DDU (DDU = двойное уплотнение)
...
- НЕ взаимозаменяем с 6206 — другой тип нагрузки и требования к монтажу
...
**Статическая грузоподъёмность** (C0): 12.5 кН (≈ 1275 кгс)
...
|---------------|-------------|------------|
| SKF | 7206 BEP | Премиум качество, Heavy duty уплотнение |
| FAG | 7206-B-TVP | Премиум, точка вместо дефиса |
| NTN | 7206B | LLU = низкое трение уплотнения |
| NSK | 7206B | DDU = двойное уплотнение |
| ГОСТ | 36206 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **7206-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **7206-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **7206** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 7205-2RS — другой d (25 мм вместо 30 мм) — не сядет на вал
- 7207-2RS — другой d (35 мм вместо 30 мм) — не сядет на вал
...
- Замена на радиальный подшипник 6206 → отказ при осевых нагрузках
