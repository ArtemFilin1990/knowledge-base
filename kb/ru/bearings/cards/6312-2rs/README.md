---
id: KB-RU-000172
title: "6312-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["6312 2rs", "6312-2RSH", "6312DDU"]
designation:
  base: "6312"
  suffixes: ["2RS"]
...
equivalents:
  - {manufacturer: "SKF", code: "6312-2RSH"}
  - {manufacturer: "FAG", code: "6312.2RSR"}
  - {manufacturer: "NTN", code: "6312LLU"}
  - {manufacturer: "NSK", code: "6312DDU"}
  - {manufacturer: "ГОСТ", code: "180312"}
load_capacity:
  dynamic_C_kN: 81.9
  static_C0_kN: 52.0
  rpm_limit: 4000
...
# 6312-2RS — подшипник шариковый радиальный
...
**Режимы работы**:
- Обороты: 400–4000 об/мин (оптимум 400–1333)
...
- SKF: 6312-2RSH (H = Heavy duty уплотнение)
- FAG: 6312.2RSR (точка вместо дефиса)
- NTN: 6312LLU (LLU = низкое трение)
- NSK: 6312DDU (DDU = двойное уплотнение)
...
**Предельные обороты**: 4000 об/мин (с уплотнениями; открытый: 5000 об/мин)
...
|---------------|-------------|------------|
| SKF | 6312-2RSH | Премиум качество, Heavy duty уплотнение |
| FAG | 6312.2RSR | Премиум, точка вместо дефиса |
| NTN | 6312LLU | LLU = низкое трение уплотнения |
| NSK | 6312DDU | DDU = двойное уплотнение |
| ГОСТ | 180312 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6312-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6312-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6312** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6212-2RS — другой D (120 мм вместо 130 мм) — не войдёт в корпус
- 6311-2RS — другой d (55 мм вместо 60 мм) — не сядет на вал
- 6313-2RS — другой d (65 мм вместо 60 мм) — не сядет на вал
...
- Применение на оборотах выше 4000 → перегрев уплотнений
