---
id: KB-RU-000053
title: "7210-2RS — подшипник шариковый радиально-упорный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["7210 2rs", "7210 BEP", "7210B"]
designation:
  base: "7210"
  suffixes: ["2RS"]
...
equivalents:
  - {manufacturer: "SKF", code: "7210 BEP"}
  - {manufacturer: "FAG", code: "7210-B-TVP"}
  - {manufacturer: "NTN", code: "7210B"}
  - {manufacturer: "NSK", code: "7210B"}
  - {manufacturer: "ГОСТ", code: "36210"}
load_capacity:
  dynamic_C_kN: 35.0
  static_C0_kN: 22.1
  rpm_limit: 5360
...
# 7210-2RS — подшипник шариковый радиально-упорный
...
- SKF: 7210-2RSH (H = Heavy duty уплотнение)
- FAG: 7210.2RSR (точка вместо дефиса)
- NTN: 7210LLU (LLU = низкое трение)
- NSK: 7210DDU (DDU = двойное уплотнение)
...
- НЕ взаимозаменяем с 6210 — другой тип нагрузки и требования к монтажу
...
**Статическая грузоподъёмность** (C0): 22.1 кН (≈ 2254 кгс)
...
|---------------|-------------|------------|
| SKF | 7210 BEP | Премиум качество, Heavy duty уплотнение |
| FAG | 7210-B-TVP | Премиум, точка вместо дефиса |
| NTN | 7210B | LLU = низкое трение уплотнения |
| NSK | 7210B | DDU = двойное уплотнение |
| ГОСТ | 36210 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **7210-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **7210-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **7210** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 7209-2RS — другой d (45 мм вместо 50 мм) — не сядет на вал
- 7211-2RS — другой d (55 мм вместо 50 мм) — не сядет на вал
...
- Замена на радиальный подшипник 6210 → отказ при осевых нагрузках
