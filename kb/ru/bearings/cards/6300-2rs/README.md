---
id: KB-RU-000036
title: "6300-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
designation:
  base: "6300"
  suffixes: ["2RS"]
dims: {d: 10, D: 35, B: 11, unit: "mm"}
standards: ["ISO 15", "ISO 281", "ГОСТ 520-2011"]
equivalents:
  - {manufacturer: "SKF", code: "6300"}
  - {manufacturer: "FAG", code: "6300"}
  - {manufacturer: "NTN", code: "6300"}
  - {manufacturer: "NSK", code: "6300"}
  - {manufacturer: "ГОСТ", code: "300"}
load_capacity:
  dynamic_C_kN: 8.06
  static_C0_kN: 3.4
  rpm_limit: 17600
...
# 6300-2RS — подшипник шариковый радиальный
...
**Базовое обозначение**: 6300
- **6** — серия шариковых радиальных однорядных (глубокий желоб)
- **3** — серия ширины средняя (B = 11 мм для этого размера)
...
- SKF: 6300-2RSH (H = Heavy duty уплотнение)
- FAG: 6300.2RSR (точка вместо дефиса)
- NTN: 6300LLU (LLU = низкое трение)
- NSK: 6300DDU (DDU = двойное уплотнение)
...
- Серия 63xx (средняя): стандартная серия для d=10 мм
- Другие зазоры: 6300-2RS C3 — размеры те же, но зазор увеличен
...
**Динамическая грузоподъёмность** (C): 8.06 кН (≈ 822 кгс)
**Статическая грузоподъёмность** (C0): 3.4 кН (≈ 347 кгс)
...
- L10 = (8060 / 400)³ × 10⁶ = 8181 × 10⁶ оборотов
- L10h = 8181 × 10⁶ / (60 × 1500) ≈ 90903 часов
...
|---------------|-------------|------------|
| SKF | 6300 | Премиум качество, Heavy duty уплотнение |
| FAG | 6300 | Премиум, точка вместо дефиса |
| NTN | 6300 | LLU = низкое трение уплотнения |
| NSK | 6300 | DDU = двойное уплотнение |
| ГОСТ | 300 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6300-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6300-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6300** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6200-2RS — другой D (25 мм вместо 35 мм) — не войдёт в корпус
- 6313-2RS — другой d (13 мм вместо 10 мм) — не сядет на вал
