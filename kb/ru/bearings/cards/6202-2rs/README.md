---
id: KB-RU-000029
title: "6202-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
equivalents:
  - {manufacturer: "SKF", code: "6202"}
  - {manufacturer: "FAG", code: "6202"}
  - {manufacturer: "NTN", code: "6202"}
  - {manufacturer: "NSK", code: "6202"}
  - {manufacturer: "ГОСТ", code: "202"}
load_capacity:
  dynamic_C_kN: 7.65
  static_C0_kN: 3.6
  rpm_limit: 16000
...
# 6202-2RS — подшипник шариковый радиальный
...
- SKF: 6202-2RSH (H = Heavy duty уплотнение)
- FAG: 6202.2RSR (точка вместо дефиса)
- NTN: 6202LLU (LLU = низкое трение)
- NSK: 6202DDU (DDU = двойное уплотнение)
...
- Серия 62xx (лёгкая): стандартная серия для d=15 мм
- Другие зазоры: 6202-2RS C3 — размеры те же, но зазор увеличен
...
**Статическая грузоподъёмность** (C0): 3.6 кН (≈ 367 кгс)
...
- Эквивалентная нагрузка P ≈ Fr = 400 Н (малая осевая)
- L10 = (7650 / 400)³ × 10⁶ = 6995 × 10⁶ оборотов
- L10h = 6995 × 10⁶ / (60 × 1500) ≈ 77725 часов
...
|---------------|-------------|------------|
| SKF | 6202 | Премиум качество, Heavy duty уплотнение |
| FAG | 6202 | Премиум, точка вместо дефиса |
| NTN | 6202 | LLU = низкое трение уплотнения |
| NSK | 6202 | DDU = двойное уплотнение |
| ГОСТ | 202 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6202-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6202-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6202** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6302-2RS — другой D (45 мм вместо 35 мм) — не войдёт в корпус
- 6213-2RS — другой d (13 мм вместо 15 мм) — не сядет на вал
- 6218-2RS — другой d (18 мм вместо 15 мм) — не сядет на вал
