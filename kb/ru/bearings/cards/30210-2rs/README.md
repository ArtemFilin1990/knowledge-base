---
id: KB-RU-000154
title: "30210-2RS — подшипник роликовый конический"
topic: bearings-card
...
designation:
  base: "30210"
  suffixes: ["2RS"]
dims: {d: 50, D: 90, B: 21.75, unit: "mm"}
standards: ["ISO 15", "ISO 281", "ГОСТ 520-2011"]
equivalents:
  - {manufacturer: "SKF", code: "30210"}
  - {manufacturer: "FAG", code: "30210"}
  - {manufacturer: "NTN", code: "30210"}
  - {manufacturer: "NSK", code: "30210"}
  - {manufacturer: "ГОСТ", code: "7210"}
load_capacity:
  dynamic_C_kN: 66.3
  static_C0_kN: 75.0
  rpm_limit: 5200
...
# 30210-2RS — подшипник роликовый конический
...
- SKF: 30210-2RSH (H = Heavy duty уплотнение)
- FAG: 30210.2RSR (точка вместо дефиса)
- NTN: 30210LLU (LLU = низкое трение)
- NSK: 30210DDU (DDU = двойное уплотнение)
...
- Серия 30xxx (): стандартная серия для d=50 мм
- Другие зазоры: 30210-2RS C3 — размеры те же, но зазор увеличен
...
**Динамическая грузоподъёмность** (C): 66.3 кН (≈ 6761 кгс)
**Статическая грузоподъёмность** (C0): 75.0 кН (≈ 7648 кгс)
**Предельные обороты**: 5200 об/мин (с уплотнениями; открытый: 6500 об/мин)
...
- L10 = (66300 / 3300)³ × 10⁶ = 22046 × 10⁶ оборотов
- L10h = 22046 × 10⁶ / (60 × 1500) ≈ 244956 часов
...
|---------------|-------------|------------|
| SKF | 30210 | Премиум качество, Heavy duty уплотнение |
| FAG | 30210 | Премиум, точка вместо дефиса |
| NTN | 30210 | LLU = низкое трение уплотнения |
| NSK | 30210 | DDU = двойное уплотнение |
| ГОСТ | 7210 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **30210-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **30210-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **30210** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
- 3009-2RS — другой d (45 мм вместо 50 мм) — не сядет на вал
- 3011-2RS — другой d (55 мм вместо 50 мм) — не сядет на вал
...
- Применение на оборотах выше 5200 → перегрев уплотнений
