---
id: KB-RU-000037
title: "6301-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
designation:
  base: "6301"
  suffixes: ["2RS"]
dims: {d: 12, D: 37, B: 12, unit: "mm"}
standards: ["ISO 15", "ISO 281", "ГОСТ 520-2011"]
equivalents:
  - {manufacturer: "SKF", code: "6301"}
  - {manufacturer: "FAG", code: "6301"}
  - {manufacturer: "NTN", code: "6301"}
  - {manufacturer: "NSK", code: "6301"}
  - {manufacturer: "ГОСТ", code: "301"}
load_capacity:
  dynamic_C_kN: 9.75
  static_C0_kN: 4.15
  rpm_limit: 15200
...
# 6301-2RS — подшипник шариковый радиальный
...
- SKF: 6301-2RSH (H = Heavy duty уплотнение)
- FAG: 6301.2RSR (точка вместо дефиса)
- NTN: 6301LLU (LLU = низкое трение)
- NSK: 6301DDU (DDU = двойное уплотнение)
...
- Серия 63xx (средняя): стандартная серия для d=12 мм
- Другие зазоры: 6301-2RS C3 — размеры те же, но зазор увеличен
...
**Динамическая грузоподъёмность** (C): 9.75 кН (≈ 994 кгс)
**Статическая грузоподъёмность** (C0): 4.15 кН (≈ 423 кгс)
**Предельные обороты**: 15200 об/мин (с уплотнениями; открытый: 19000 об/мин)
...
- L10 = (9750 / 500)³ × 10⁶ = 7414 × 10⁶ оборотов
- L10h = 7414 × 10⁶ / (60 × 1500) ≈ 82387 часов
...
|---------------|-------------|------------|
| SKF | 6301 | Премиум качество, Heavy duty уплотнение |
| FAG | 6301 | Премиум, точка вместо дефиса |
| NTN | 6301 | LLU = низкое трение уплотнения |
| NSK | 6301 | DDU = двойное уплотнение |
| ГОСТ | 301 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6301-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6301-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6301** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6201-2RS — другой D (27 мм вместо 37 мм) — не войдёт в корпус
- 6300-2RS — другой d (10 мм вместо 12 мм) — не сядет на вал
- 6302-2RS — другой d (15 мм вместо 12 мм) — не сядет на вал
...
- Применение на оборотах выше 15200 → перегрев уплотнений
