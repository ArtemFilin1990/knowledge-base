---
id: KB-RU-000073
title: "NJ210-2RS — подшипник роликовый цилиндрический"
topic: bearings-card
...
equivalents:
  - {manufacturer: "SKF", code: "NJ210"}
  - {manufacturer: "FAG", code: "NJ210"}
  - {manufacturer: "NTN", code: "NJ210"}
  - {manufacturer: "NSK", code: "NJ210"}
  - {manufacturer: "ГОСТ", code: "42310"}
load_capacity:
  dynamic_C_kN: 65.5
  static_C0_kN: 58.0
  rpm_limit: 5360
...
# NJ210-2RS — подшипник роликовый цилиндрический
...
**Базовое обозначение**: NJ210
- **N** — серия роликовых цилиндрических
- **J** — серия ширины  (B = 20 мм для этого размера)
- **210** — код внутреннего диаметра: d = 10 × 5 = 50 мм
...
- SKF: NJ210-2RSH (H = Heavy duty уплотнение)
- FAG: NJ210.2RSR (точка вместо дефиса)
- NTN: NJ210LLU (LLU = низкое трение)
- NSK: NJ210DDU (DDU = двойное уплотнение)
...
- Серия NJxx (): стандартная серия для d=50 мм
- Другие зазоры: NJ210-2RS C3 — размеры те же, но зазор увеличен
...
**Динамическая грузоподъёмность** (C): 65.5 кН (≈ 6679 кгс)
**Статическая грузоподъёмность** (C0): 58.0 кН (≈ 5914 кгс)
...
**Пример расчёта**:
- Радиальная нагрузка Fr = 3300 Н
- Частота вращения n = 1500 об/мин
- Эквивалентная нагрузка P ≈ Fr = 3300 Н (малая осевая)
- L10 = (65500 / 3300)³ × 10⁶ = 21171 × 10⁶ оборотов
- L10h = 21171 × 10⁶ / (60 × 1500) ≈ 235242 часов
...
|---------------|-------------|------------|
| SKF | NJ210 | Премиум качество, Heavy duty уплотнение |
| FAG | NJ210 | Премиум, точка вместо дефиса |
| NTN | NJ210 | LLU = низкое трение уплотнения |
| NSK | NJ210 | DDU = двойное уплотнение |
| ГОСТ | 42310 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **NJ210-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **NJ210-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **NJ210** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 62210-2RS — другой D (90 мм вместо 90 мм) — не войдёт в корпус
- NJ09-2RS — другой d (45 мм вместо 50 мм) — не сядет на вал
- NJ11-2RS — другой d (55 мм вместо 50 мм) — не сядет на вал
