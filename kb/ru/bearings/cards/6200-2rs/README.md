---
id: KB-RU-000027
title: "6200-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
equivalents:
  - {manufacturer: "SKF", code: "6200"}
  - {manufacturer: "FAG", code: "6200"}
  - {manufacturer: "NTN", code: "6200"}
  - {manufacturer: "NSK", code: "6200"}
  - {manufacturer: "ГОСТ", code: "200"}
load_capacity:
  dynamic_C_kN: 5.07
  static_C0_kN: 2.36
  rpm_limit: 20800
...
# 6200-2RS — подшипник шариковый радиальный
...
- SKF: 6200-2RSH (H = Heavy duty уплотнение)
- FAG: 6200.2RSR (точка вместо дефиса)
- NTN: 6200LLU (LLU = низкое трение)
- NSK: 6200DDU (DDU = двойное уплотнение)
...
- Серия 62xx (лёгкая): стандартная серия для d=10 мм
- Другие зазоры: 6200-2RS C3 — размеры те же, но зазор увеличен
...
**Статическая грузоподъёмность** (C0): 2.36 кН (≈ 241 кгс)
...
- L10 = (5070 / 300)³ × 10⁶ = 4826 × 10⁶ оборотов
- L10h = 4826 × 10⁶ / (60 × 1500) ≈ 53631 часов
...
|---------------|-------------|------------|
| SKF | 6200 | Премиум качество, Heavy duty уплотнение |
| FAG | 6200 | Премиум, точка вместо дефиса |
| NTN | 6200 | LLU = низкое трение уплотнения |
| NSK | 6200 | DDU = двойное уплотнение |
| ГОСТ | 200 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6200-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6200-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6200** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6300-2RS — другой D (40 мм вместо 30 мм) — не войдёт в корпус
- 6213-2RS — другой d (13 мм вместо 10 мм) — не сядет на вал
...
- Применение на оборотах выше 20800 → перегрев уплотнений
