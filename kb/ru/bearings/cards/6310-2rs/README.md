---
id: KB-RU-000055
title: "6310-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["6310 2rs", "6310-2RSH", "6310DDU"]
designation:
  base: "6310"
  suffixes: ["2RS"]
dims: {d: 50, D: 110, B: 27, unit: "mm"}
standards: ["ISO 15", "ISO 281", "ГОСТ 520-2011"]
equivalents:
  - {manufacturer: "SKF", code: "6310-2RSH"}
  - {manufacturer: "FAG", code: "6310.2RSR"}
  - {manufacturer: "NTN", code: "6310LLU"}
  - {manufacturer: "NSK", code: "6310DDU"}
  - {manufacturer: "ГОСТ", code: "180310"}
load_capacity:
  dynamic_C_kN: 61.8
  static_C0_kN: 34.0
  rpm_limit: 4800
...
# 6310-2RS — подшипник шариковый радиальный
...
- **3** — серия ширины средняя (B = 27 мм для этого размера)
...
- SKF: 6310-2RSH (H = Heavy duty уплотнение)
- FAG: 6310.2RSR (точка вместо дефиса)
- NTN: 6310LLU (LLU = низкое трение)
- NSK: 6310DDU (DDU = двойное уплотнение)
...
|---------------|-------------|------------|
| SKF | 6310-2RSH | Премиум качество, Heavy duty уплотнение |
| FAG | 6310.2RSR | Премиум, точка вместо дефиса |
| NTN | 6310LLU | LLU = низкое трение уплотнения |
| NSK | 6310DDU | DDU = двойное уплотнение |
| ГОСТ | 180310 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6310-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6310-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6310** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6210-2RS — другой D (100 мм вместо 110 мм) — не войдёт в корпус
- 6309-2RS — другой d (45 мм вместо 50 мм) — не сядет на вал
- 6311-2RS — другой d (55 мм вместо 50 мм) — не сядет на вал
