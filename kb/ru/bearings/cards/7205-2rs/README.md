---
id: KB-RU-000048
title: "7205-2RS — подшипник шариковый радиально-упорный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["7205 2rs", "7205 BEP", "7205B"]
designation:
  base: "7205"
  suffixes: ["2RS"]
...
# 7205-2RS — подшипник шариковый радиально-упорный
...
- SKF: 7205-2RSH (H = Heavy duty уплотнение)
- FAG: 7205.2RSR (точка вместо дефиса)
- NTN: 7205LLU (LLU = низкое трение)
- NSK: 7205DDU (DDU = двойное уплотнение)
...
**Динамическая грузоподъёмность** (C): 14.0 кН (≈ 1428 кгс)
**Статическая грузоподъёмность** (C0): 8.5 кН (≈ 867 кгс)
...
|---------------|-------------|------------|
| SKF | 7205 BEP | Премиум качество, Heavy duty уплотнение |
| FAG | 7205-B-XL-TVP | Премиум, точка вместо дефиса |
| NTN | 7205B | LLU = низкое трение уплотнения |
| NSK | 7205B | DDU = двойное уплотнение |
| ГОСТ | 46205 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **7205-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **7205-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **7205** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 7204-2RS — другой d (20 мм вместо 25 мм) — не сядет на вал
- 7206-2RS — другой d (30 мм вместо 25 мм) — не сядет на вал
