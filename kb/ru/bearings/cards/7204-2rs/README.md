---
id: KB-RU-000047
title: "7204-2RS — подшипник шариковый радиально-упорный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["7204 2rs", "7204 BEP", "7204B"]
designation:
  base: "7204"
  suffixes: ["2RS"]
...
equivalents:
  - {manufacturer: "SKF", code: "7204 BEP"}
  - {manufacturer: "FAG", code: "7204-B-TVP"}
  - {manufacturer: "NTN", code: "7204B"}
  - {manufacturer: "NSK", code: "7204B"}
  - {manufacturer: "ГОСТ", code: "36204"}
load_capacity:
  dynamic_C_kN: 13.5
  static_C0_kN: 7.35
  rpm_limit: 12000
...
# 7204-2RS — подшипник шариковый радиально-упорный
...
- SKF: 7204-2RSH (H = Heavy duty уплотнение)
- FAG: 7204.2RSR (точка вместо дефиса)
- NTN: 7204LLU (LLU = низкое трение)
- NSK: 7204DDU (DDU = двойное уплотнение)
...
- НЕ взаимозаменяем с 6204 — другой тип нагрузки и требования к монтажу
...
**Статическая грузоподъёмность** (C0): 7.35 кН (≈ 749 кгс)
...
|---------------|-------------|------------|
| SKF | 7204 BEP | Премиум качество, Heavy duty уплотнение |
| FAG | 7204-B-TVP | Премиум, точка вместо дефиса |
| NTN | 7204B | LLU = низкое трение уплотнения |
| NSK | 7204B | DDU = двойное уплотнение |
| ГОСТ | 36204 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **7204-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **7204-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **7204** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 7202-2RS — другой d (15 мм вместо 20 мм) — не сядет на вал
- 7205-2RS — другой d (25 мм вместо 20 мм) — не сядет на вал
...
- Замена на радиальный подшипник 6204 → отказ при осевых нагрузках
