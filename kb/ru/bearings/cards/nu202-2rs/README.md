---
id: KB-RU-000056
title: "NU202-2RS — подшипник роликовый цилиндрический"
topic: bearings-card
...
equivalents:
  - {manufacturer: "SKF", code: "NU202"}
  - {manufacturer: "FAG", code: "NU202"}
  - {manufacturer: "NTN", code: "NU202"}
  - {manufacturer: "NSK", code: "NU202"}
  - {manufacturer: "ГОСТ", code: "32302"}
load_capacity:
  dynamic_C_kN: 13.5
  static_C0_kN: 10.4
  rpm_limit: 16000
...
# NU202-2RS — подшипник роликовый цилиндрический
...
**Базовое обозначение**: NU202
- **N** — серия роликовых цилиндрических
- **U** — серия ширины  (B = 11 мм для этого размера)
...
- SKF: NU202-2RSH (H = Heavy duty уплотнение)
- FAG: NU202.2RSR (точка вместо дефиса)
- NTN: NU202LLU (LLU = низкое трение)
- NSK: NU202DDU (DDU = двойное уплотнение)
...
- Серия NUxx (): стандартная серия для d=15 мм
- Другие зазоры: NU202-2RS C3 — размеры те же, но зазор увеличен
...
**Статическая грузоподъёмность** (C0): 10.4 кН (≈ 1061 кгс)
...
- L10 = (13500 / 700)³ × 10⁶ = 19236 × 10⁶ оборотов
...
|---------------|-------------|------------|
| SKF | NU202 | Премиум качество, Heavy duty уплотнение |
| FAG | NU202 | Премиум, точка вместо дефиса |
| NTN | NU202 | LLU = низкое трение уплотнения |
| NSK | NU202 | DDU = двойное уплотнение |
| ГОСТ | 32302 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **NU202-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **NU202-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **NU202** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
- NU13-2RS — другой d (13 мм вместо 15 мм) — не сядет на вал
- NU18-2RS — другой d (18 мм вместо 15 мм) — не сядет на вал
...
- Применение на оборотах выше 16000 → перегрев уплотнений
