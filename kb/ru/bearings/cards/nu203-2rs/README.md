---
id: KB-RU-000057
title: "NU203-2RS — подшипник роликовый цилиндрический"
topic: bearings-card
...
equivalents:
  - {manufacturer: "SKF", code: "NU203"}
  - {manufacturer: "FAG", code: "NU203"}
  - {manufacturer: "NTN", code: "NU203"}
  - {manufacturer: "NSK", code: "NU203"}
  - {manufacturer: "ГОСТ", code: "32303"}
load_capacity:
  dynamic_C_kN: 17.1
  static_C0_kN: 13.5
  rpm_limit: 14400
...
# NU203-2RS — подшипник роликовый цилиндрический
...
**Базовое обозначение**: NU203
- **N** — серия роликовых цилиндрических
- **U** — серия ширины  (B = 12 мм для этого размера)
...
- SKF: NU203-2RSH (H = Heavy duty уплотнение)
- FAG: NU203.2RSR (точка вместо дефиса)
- NTN: NU203LLU (LLU = низкое трение)
- NSK: NU203DDU (DDU = двойное уплотнение)
...
- Серия NUxx (): стандартная серия для d=17 мм
- Другие зазоры: NU203-2RS C3 — размеры те же, но зазор увеличен
...
|---------------|-------------|------------|
| SKF | NU203 | Премиум качество, Heavy duty уплотнение |
| FAG | NU203 | Премиум, точка вместо дефиса |
| NTN | NU203 | LLU = низкое трение уплотнения |
| NSK | NU203 | DDU = двойное уплотнение |
| ГОСТ | 32303 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **NU203-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **NU203-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **NU203** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
- NU02-2RS — другой d (15 мм вместо 17 мм) — не сядет на вал
- NU04-2RS — другой d (20 мм вместо 17 мм) — не сядет на вал
