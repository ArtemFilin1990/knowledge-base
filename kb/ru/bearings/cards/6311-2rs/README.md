---
id: KB-RU-000171
title: "6311-2RS — подшипник шариковый радиальный"
topic: bearings-card
...
created: 2026-02-06
updated: 2026-02-06
aliases: ["6311 2rs", "6311-2RSH", "6311DDU"]
designation:
  base: "6311"
  suffixes: ["2RS"]
dims: {d: 55, D: 120, B: 29.0, unit: "mm"}
standards: ["ISO 15", "ISO 281", "ГОСТ 520-2011"]
equivalents:
  - {manufacturer: "SKF", code: "6311-2RSH"}
  - {manufacturer: "FAG", code: "6311.2RSR"}
  - {manufacturer: "NTN", code: "6311LLU"}
  - {manufacturer: "NSK", code: "6311DDU"}
  - {manufacturer: "ГОСТ", code: "180311"}
load_capacity:
  dynamic_C_kN: 71.5
  static_C0_kN: 45.5
  rpm_limit: 4240
...
# 6311-2RS — подшипник шариковый радиальный
...
**Базовое обозначение**: 6311
- **6** — серия шариковых радиальных однорядных (глубокий желоб)
- **3** — серия ширины средняя (B = 29.0 мм для этого размера)
...
- SKF: 6311-2RSH (H = Heavy duty уплотнение)
- FAG: 6311.2RSR (точка вместо дефиса)
- NTN: 6311LLU (LLU = низкое трение)
- NSK: 6311DDU (DDU = двойное уплотнение)
...
- Серия 63xx (средняя): стандартная серия для d=55 мм
- Другие зазоры: 6311-2RS C3 — размеры те же, но зазор увеличен
...
**Динамическая грузоподъёмность** (C): 71.5 кН (≈ 7291 кгс)
...
- L10 = (71500 / 3600)³ × 10⁶ = 7834 × 10⁶ оборотов
- L10h = 7834 × 10⁶ / (60 × 1500) ≈ 87049 часов
...
|---------------|-------------|------------|
| SKF | 6311-2RSH | Премиум качество, Heavy duty уплотнение |
| FAG | 6311.2RSR | Премиум, точка вместо дефиса |
| NTN | 6311LLU | LLU = низкое трение уплотнения |
| NSK | 6311DDU | DDU = двойное уплотнение |
| ГОСТ | 180311 | Отечественный, код "18" = двухстороннее уплотнение |

**Функциональные аналоги** (допустимые отличия):
- **6311-2RS C3**: увеличенный зазор — для работы при повышенных температурах
- **6311-2Z**: металлические шайбы вместо резины — меньшая защита, выше обороты
- **6311** (открытый): нет уплотнений — требуется внешняя защита, выше обороты
...
**Нельзя заменять на**:
- 6211-2RS — другой D (110 мм вместо 120 мм) — не войдёт в корпус
- 6310-2RS — другой d (50 мм вместо 55 мм) — не сядет на вал
- 6312-2RS — другой d (60 мм вместо 55 мм) — не сядет на вал
