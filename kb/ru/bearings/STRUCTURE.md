...
└── cards/                      # Карточки конкретных подшипников
    ...
    ├── 6302-2rs/              # Серия 63xx
    ├── 6304-2rs/              # Серия 63xx
    ├── 6305-2rs-c3/           # Серия 63xx (пример)
    └── 7205-b/                # Серия 7xxx (радиально-упорный)
```
...
- Последовательная нумерация (текущий максимум: KB-RU-000025)
...
- **Статей**: 25 (13 в разделе bearings, 6 карточек, 2 ISO-справочника, 4 в других разделах)
...
3. ✅ Интеграция структурированных данных из каталогов (CSV)
4. ✅ Автоматизация генерации карточек из CSV (`scripts/generate_bearing_cards.py`)
5. ✅ Визуальные диаграммы в разделе failure-analysis (Mermaid)

### Планы развития

1. Расширение каталога карточек (все типоразмеры серий 62xx, 63xx, 7xxx)
2. Добавление карточек роликовых подшипников (серии NU, NJ, конические)
3. Интеграция прайс-листов в автоматическую генерацию
4. Верификация данных для перевода из draft в verified
