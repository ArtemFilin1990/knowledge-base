Process one inbox batch.

1) Scan inbox/ (ignore inbox/processed/).
2) For each new file: extract text, choose topic slug (ascii kebab-case), split into 1..N articles.
...
6) Dedup: if content sha256 already in _meta/dedup_index.json, do NOT create duplicate; log in _meta/dedup_log.md.
